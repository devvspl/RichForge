...
var n = (l, t, e) => g(l, typeof t != "symbol" ? t + "" : t, e);
class f {
  constructor(t) {
    n(this, "core");
    n(this, "element");
    this.core = t, this.element = document.createElement("div"), this.element.className = "rf-toolbar", this.render();
  }
  render() {
    this.element.innerHTML = "";
    const t = this.core.options.toolbar || [], e = {}, s = (i) => {
      if (!e[i]) {
        const o = document.createElement("div");
        o.className = "rf-toolbar-group", this.element.appendChild(o), e[i] = o;
      }
      return e[i];
    };
    t.forEach((i) => {
  ...
  addButton(t, e, s, i, o) {
    ...
    r.type = "button", r.className = "rf-btn", r.title = s, r.innerHTML = i, r.addEventListener("click", (a) => {
      a.preventDefault(), o();
    }), t.appendChild(r);
  }
  addFormatButton(t, e, s, i, o, r) {
    const a = document.createElement("button");
    a.type = "button", a.className = "rf-btn", a.dataset.command = e, a.title = s, a.innerHTML = i, a.addEventListener("click", (c) => {
      c.preventDefault(), this.core.exec(o || e, r);
    }), t.appendChild(a);
  }
  addHeadingSelect(t) {
    const e = document.createElement("select");
    e.className = "rf-select", e.innerHTML = `
...
function h(l) {
  if (!l) return "";
  const e = new DOMParser().parseFromString(l, "text/html");
  return v(e.body), e.body.innerHTML;
}
function v(l) {
  const t = Array.from(l.childNodes);
  for (const e of t)
    if (e.nodeType === Node.ELEMENT_NODE) {
      const s = e, i = s.tagName.toLowerCase();
      if (!E.has(i))
        if (["script", "style", "object", "embed", "form"].includes(i)) {
          s.remove();
          continue;
        } else {
      ...
      for (const r of o) {
        ...
        if (a.startsWith("on") || !b.has(a)) {
          s.removeAttribute(r.name);
          continue;
        }
        if (["href", "src"].includes(a)) {
          const c = r.value.trim().toLowerCase();
          (c.startsWith("javascript:") || c.startsWith("vbscript:")) && s.removeAttribute(r.name);
        }
      }
      v(s);
    }
...
const p = class p {
  constructor(t, e = {}) {
    ...
    n(this, "statusbarEl");
    n(this, "wordCountEl");
    n(this, "isSourceView", !1);
    n(this, "isFullscreen", !1);
    if (typeof t == "string") {
      const s = document.querySelector(t);
      if (!s)
        throw new Error(`RichForge: Target element "${t}" not found.`);
  ...
  init() {
    ...
    const t = typeof this.options.height == "number" ? `${this.options.height}px` : this.options.height;
    t && (this.contentEl.style.minHeight = t);
    let e = "";
    this.targetElement instanceof HTMLTextAreaElement || this.targetElement instanceof HTMLInputElement ? e = this.targetElement.value : e = this.targetElement.innerHTML, this.contentEl.innerHTML = h(e), this.wrapperEl.appendChild(this.contentEl), this.sourceEl = document.createElement("textarea"), this.sourceEl.className = "rf-source-textarea", this.sourceEl.style.display = "none", t && (this.sourceEl.style.minHeight = t), this.wrapperEl.appendChild(this.sourceEl), this.statusbarEl = document.createElement("div"), this.statusbarEl.className = "rf-statusbar", this.statusbarEl.innerHTML = `
  ...
  bindEvents() {
    ...
    }), this.contentEl.addEventListener("dragover", (t) => t.preventDefault()), this.contentEl.addEventListener("drop", (t) => {
      t.preventDefault(), t.dataTransfer && t.dataTransfer.files && t.dataTransfer.files.length > 0 && this.handleFileUpload(t.dataTransfer.files[0]);
    }), this.contentEl.addEventListener("paste", (t) => {
      if (t.clipboardData && t.clipboardData.items) {
        for (const e of Array.from(t.clipboardData.items))
          if (e.type.indexOf("image") !== -1) {
            const s = e.getAsFile();
            if (s) {
              t.preventDefault(), this.handleFileUpload(s);
              break;
            }
          }
      }
    }), this.sourceEl.addEventListener("input", () => {
      this.contentEl.innerHTML = h(this.sourceEl.value), this.syncToTarget(), this.options.onChange && this.options.onChange(this.getHTML());
    });
  }
  exec(t, e = "") {
    this.contentEl.focus(), document.execCommand(t, !1, e), this.syncToTarget(), this.updateWordCount(), this.options.onChange && this.options.onChange(this.getHTML());
  }
  syncToTarget() {
    const t = this.getHTML();
    this.targetElement instanceof HTMLTextAreaElement || this.targetElement instanceof HTMLInputElement ? this.targetElement.value = t : this.targetElement.innerHTML = t;
  }
  updateWordCount() {
    const e = (this.contentEl.innerText || "").trim(), s = e ? e.split(/\s+/).length : 0, i = e.length;
    this.wordCountEl.textContent = `${s} words | ${i} chars`;
  }
  getHTML() {
    return this.isSourceView ? h(this.sourceEl.value) : h(this.contentEl.innerHTML);
  }
  getText() {
  ...
  setHTML(t) {
    const e = h(t);
    this.contentEl.innerHTML = e, this.sourceEl.value = e, this.syncToTarget(), this.updateWordCount();
  }
  getData() {
    return this.getHTML();
  }
  ...
  toggleSourceView() {
    this.isSourceView = !this.isSourceView, this.isSourceView ? (this.sourceEl.value = this.getHTML(), this.contentEl.style.display = "none", this.sourceEl.style.display = "block") : (this.contentEl.innerHTML = h(this.sourceEl.value), this.sourceEl.style.display = "none", this.contentEl.style.display = "block");
  }
  toggleFullscreen() {
  ...
  handleFileUpload(t) {
    var i;
    if (!((i = this.options.upload) != null && i.enabled)) {
      alert("Upload feature is disabled for this editor.");
      return;
    }
    const e = new FormData();
    e.append("file", t), e.append("project_key", this.options.projectKey || "");
    ...
    fetch(s, {
      ...
      if (o.success && o.data && o.data.url)
        t.type.startsWith("image/") ? this.exec("insertImage", o.data.url) : this.exec("createLink", o.data.url), this.options.onUpload && this.options.onUpload(o.data);
      else {
        const r = o.message || "File upload failed.";
        this.options.onError ? this.options.onError({ code: "UPLOAD_FAILED", message: r }) : alert(`RichForge Upload Error: ${r}`);
      }
    }).catch((o) => {
      const r = "RichForge could not connect to the upload service.";
      this.options.onError ? this.options.onError({ code: "NETWORK_ERROR", message: r }) : alert(r);
    });
  }
  promptInsertLink() {
    const t = prompt("Enter Destination URL:", "https://");
    t && this.exec("createLink", t);
  }
  promptInsertImage() {
    if (confirm("Click OK to upload an image from your device, or Cancel to enter an Image URL.")) {
      const e = document.createElement("input");
      e.type = "file", e.accept = "image/*", e.onchange = (s) => {
        const i = s.target;
        i.files && i.files[0] && this.handleFileUpload(i.files[0]);
      }, e.click();
    } else {
      const e = prompt("Enter Image URL:");
      e && this.exec("insertImage", e);
    }
  }
  promptInsertTable() {
    const t = parseInt(prompt("Number of rows:", "3") || "0", 10), e = parseInt(prompt("Number of columns:", "3") || "0", 10);
    if (t > 0 && e > 0) {
      let s = '<table border="1"><tbody>';
      for (let i = 0; i < t; i++) {
        s += "<tr>";
        for (let o = 0; o < e; o++)
          s += i === 0 ? "<th>Header</th>" : "<td>Cell</td>";
        s += "</tr>";
      }
...
typeof window < "u" && (window.RichForge = u, window.RichForgeEditor = u);
export {
  u as RichForge,
  d as RichForgeCore,
  u as default
