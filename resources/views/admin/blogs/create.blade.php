...
@section('title', 'Create Blog Post - Admin Console')
@section('header_title', 'Create New Blog Post')
...
<div class="max-w-7xl mx-auto space-y-6" x-data="blogPostForm()">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-2 border-b border-slate-200">
        <div>
            <h2 class="text-2xl font-serif font-extrabold text-[#1a1a2e]">Create New Article</h2>
        </div>
                ...
                <div class="bg-white border border-slate-200 rounded-2xl p-6 sm:p-8 shadow-sm space-y-6">
                    ...
                    <div class="space-y-4">
                        <div class="space-y-1.5">
                            ...
                            <input type="text"
                                   ...
                                   @input="updateSlug()"
                                   required
                                   placeholder="e.g. How to Embed a WYSIWYG Editor into Laravel"
                    ...
                    <div class="space-y-1.5">
                        ...
                        <textarea name="excerpt"
                                  ...
                                  class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-[#1a1a2e] focus:ring-2 focus:ring-fuchsia-500">{{ old('excerpt') }}</textarea>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="content"
                                  ...
                                  rows="14"
                                  required
                                  placeholder="Write your full article content using Markdown or HTML formatting..."
                                  class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-xs font-mono text-[#1a1a2e] focus:ring-2 focus:ring-fuchsia-500">{{ old('content') }}</textarea>
                        ...
                        <div x-show="content.length > 50 && !hasHeadings" x-transition class="p-3 bg-amber-50 border border-amber-200 rounded-xl text-xs text-amber-800 flex items-center gap-2">
                            ...
                            <span><strong>SEO Suggestion:</strong> No H2 or H3 headings (`## Heading`) detected. Consider adding headings to structure long sections for search engines.</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-4">
                    ...
                    <div class="space-y-3">
                        <div class="relative border-2 border-dashed border-slate-300 hover:border-fuchsia-500 rounded-xl p-4 text-center bg-slate-50 transition-colors cursor-pointer">
                            ...
                            <span class="text-xs font-semibold text-slate-700 block">Upload Image File</span>
...
<script>
    function blogPostForm() {
        return {
            ...
            focusKeyword: @json(old('focus_keyword', '')),
            canonicalUrl: @json(old('canonical_url', '')),
            robots: @json(old('robots', 'index, follow')),
            featuredImageUrl: @json(old('featured_image', '')),
            featuredImageAlt: @json(old('featured_image_alt', '')),
            featuredImagePreview: null,
            ogTitle: @json(old('og_title', '')),
            ogDescription: @json(old('og_description', '')),
            ogImagePreview: null,
            ...
            isPublished: true,
            isCustomDate: true,
            publishedAt: @json(old('published_at', now()->format('Y-m-d\TH:i'))),
            copied: false,
